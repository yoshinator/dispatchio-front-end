...
class Job extends Component {
  ...
  handleSubmit = (e) => {
    ...
    JOBAdapter.updateItem(body, this.state.id)
      .then(response => {
        if (response.ok) {
  ...
  selectionOptionStatus =() => {
    switch (this.state.status) {
      case "IR in route":
        return (<><option selected="selected">IR in route</option><option>IP in progress</option><option>DN done</option><option>FU follow up</option></>)
      case "IP in progress":
        return (<><option >IR in route</option><option selected="selected">IP in progress</option><option>DN done</option><option>FU follow up</option></>)
      case "DN done":
        return (<><option >IR in route</option><option>IP in progress</option><option selected="selected">DN done</option><option>FU follow up</option></>)
      case "FU follow up":
        return (<><option >IR in route</option><option>IP in progress</option><option>DN done</option><option selected="selected">FU follow up</option></>)
      default: 
        return (<><option>IR in route</option> <option>IP in progress</option> <option>DN done</option> <option>FU follow up</option></>)
    }
  }
  ...
  selectionOptionPayment = () => {
      switch (this.state.payment_type) {
        ...
        return (<>
          ...
          case "CK Check":
          return <><option >CC Credit Card</option>
            <option>CH Cash</option>
          ...
          return <><option >CC Credit Card</option>
            ...
            <option>INV Invoice</option></>
          default: 
          return <><option selected="selected">CC Credit Card</option>
            <option>CH Cash</option>
            <option>CK Check</option>
            <option>INV Invoice</option></>
      }
    ...
    return <>
        ...
        <div className="card inner-card">
          <h2 className="job-edit-title">
            {this.props.job.city}, {this.props.job.customer.name} {this.props.job.customer.phone}, {this.props.job.status}
          </h2>
          <div className="card-body">
            <form onSubmit={this.handleSubmit} autoComplete="one">
              ...
              <div className="form-group">
                ...
                <select onChange={this.handleChange} className="form-control" name="status" id={`jobStatusSelect${this.props.job.id}`} value={this.state.status}>
              ...
              <div className="form-group">
                ...
                <select onChange={this.handleChange} className="form-control" name="status" id={`jobPaymentSelect${this.props.job.id}`} value={this.state.payment_type}>
